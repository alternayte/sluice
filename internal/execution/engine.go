...
type EndHook func(ctx context.Context, tx pgx.Tx, exec dbq.Execution) error
...
func (e *Engine) start(ctx context.Context, tx pgx.Tx, ex dbq.Execution, def *Definition) error {
	q := dbq.New(tx)
	now := e.Clock.Now()
	...
	if err := q.StartExecution(ctx, dbq.StartExecutionParams{ID: ex.ID, StartedAt: &now, DeadlineAt: deadline}); err != nil {
		return err
	}
	...
	for _, t := range def.Flow.Tasks {
		...
		cfg := def.Config(t)
		id, _ := uuid.NewV7()
		if err := q.InsertTaskRun(ctx, dbq.InsertTaskRunParams{ID: id, ExecutionID: ex.ID, TaskKey: t.ID, TaskType: t.Type, Attempt: 1,
...
func dependencyDecision(runIf string, deps []dbq.TaskRun) (string, string) {
...
func (e *Engine) advance(ctx context.Context, tx pgx.Tx, execID uuid.UUID) error {
	...
	for changed := true; changed; {
		...
		for _, t := range def.Flow.Tasks {
			...
			if decision == "skip" {
				if _, err := q.SkipTaskRun(ctx, dbq.SkipTaskRunParams{ID: tr.ID, Reason: reason, EndedAt: &now}); err != nil {
					return err
				}
				changed = true
				continue
			...
			if _, err := q.QueueTaskRun(ctx, dbq.QueueTaskRunParams{ID: tr.ID, QueuedAt: &now}); err != nil {
				return err
			}
			active++
			changed = true
...
func (e *Engine) finalize(ctx context.Context, tx pgx.Tx, ex dbq.Execution, def *Definition, latest map[string]dbq.TaskRun) error {
	q := dbq.New(tx)
	now := e.Clock.Now()
	...
	n, err := q.SetExecutionState(ctx, dbq.SetExecutionStateParams{ToState: to, Reason: reason, Error: errText, EndedAt: &now,
...
func (e *Engine) resolveFlowOutputs(ctx context.Context, ex dbq.Execution, def *Definition, latest map[string]dbq.TaskRun) (map[string]any, error) {
...
func (e *Engine) finishTaskTx(ctx context.Context, tx pgx.Tx, taskRunID uuid.UUID, fromStates []string, state, reason, errText string, exitCode *int) error {
	...
	if _, err := q.FinishTaskRun(ctx, dbq.FinishTaskRunParams{ToState: state, Reason: reason, Error: truncate(errText, 4000), ExitCode: ec,
	...
	if (state == TaskFailed || state == TaskTimedOut) && ex.State == ExecRunning && ex.Reason != ReasonTimeout {
		...
		if t, ok := def.Task(tr.TaskKey); ok {
			...
			if int(tr.Attempt) < cfg.Retry.MaxAttempts {
				...
				id, _ := uuid.NewV7()
				if err := q.InsertTaskRun(ctx, dbq.InsertTaskRunParams{ID: id, ExecutionID: tr.ExecutionID, TaskKey: tr.TaskKey, TaskType: tr.TaskType,
...
func (e *Engine) cancelTx(ctx context.Context, tx pgx.Tx, id uuid.UUID, record bool) error {
	...
	switch ex.State {
	case ExecQueued:
		if _, err := q.SetExecutionState(ctx, dbq.SetExecutionStateParams{ToState: ExecCancelled, Reason: ReasonCancelled, EndedAt: &now,
			ID: id, FromState: ExecQueued}); err != nil {
			return err
		}
		if _, err := q.CancelPendingTaskRuns(ctx, dbq.CancelPendingTaskRunsParams{ExecutionID: id, EndedAt: &now}); err != nil {
			return err
		}
	...
	case ExecRunning:
		if _, err := q.SetExecutionState(ctx, dbq.SetExecutionStateParams{ToState: ExecCancelling, Reason: ReasonCancelled, ID: id, FromState: ExecRunning}); err != nil {
			return err
		}
...
func (e *Engine) stopTasks(ctx context.Context, tx pgx.Tx, execID uuid.UUID) error {
	q := dbq.New(tx)
	now := e.Clock.Now()
	if _, err := q.CancelPendingTaskRuns(ctx, dbq.CancelPendingTaskRunsParams{ExecutionID: execID, EndedAt: &now}); err != nil {
		return err
	}
...
func (e *Engine) subflowParentEnd(ctx context.Context, tx pgx.Tx, child dbq.Execution) error {
	...
	if child.State == ExecSuccess {
		...
		if err := q.MergeTaskOutputs(ctx, dbq.MergeTaskOutputsParams{ID: parent.ID, Outputs: outputs}); err != nil {
			return err
		}
