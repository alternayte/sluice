...
import { AlertTriangle, CheckCircle2, CornerLeftUp, Plus, XCircle } from "lucide-react";
...
export function SecretsPanel({ namespace }: { namespace?: string }) {
  ...
  const globalSecrets = useQuery({ ...listGlobalSecretsOptions(), enabled: !namespace, select: (d) => d.items });
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <Dialog open={editing !== null} onClose={() => setEditing(null)} title={editing === "new" ? "Add secret" : "Edit secret"}>
        {editing !== null && (
          <SecretForm namespace={namespace} secret={editing === "new" ? undefined : editing} onDone={() => setEditing(null)} />
        )}
      </Dialog>
...
function SecretForm({ namespace, secret, onDone }: { namespace?: string; secret?: SecretInfo; onDone: () => void }) {
  ...
  const putNamespace = useMutation({ ...putNamespaceSecretMutation(), onSuccess });
    ...
    <form onSubmit={submit} className="flex flex-col gap-3">
      ...
      {isBuiltin ? (
        <Field
          id="secret-value"
          label="Value"
          hint={secret ? "Leave empty to keep the stored value. The value is never shown again." : "The value is never shown again."}
          error={fields.value}
        >
          <Input
            type="password"
        ...
        <Field
          ...
          error={fields.ref}
        >
          <Input required={type !== "env"} maxLength={512} value={ref} onChange={(e) => setRef(e.target.value)} className="font-mono" />
        </Field>
      )}
...
function SecretDelete({ namespace, secret, onDone }: { namespace?: string; secret: SecretInfo; onDone: () => void }) {
  ...
  const delNamespace = useMutation({ ...deleteNamespaceSecretMutation(), onSuccess });
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-sm">
        Delete <span className="font-mono">{secret.key}</span> from {secret.scope}? Flows then get the value of a parent scope, or fail with
        secret_not_found.
      </p>
      {mutation.isError && <FormError>{errorMessage(mutation.error)}</FormError>}
      <div className="flex justify-end gap-2">
